...
import { Link } from '../types/application/link';
...
async function getLink(url: string): Link {
    const id =  parseURL(url);
    const sizes: FlickrSizes = (await getSizes(id))['data'];
    const originalSize: FlickrSize = find(sizes, size => {
    ...
    return {
        ...
        creation_date: new Date(information.dates.posted),
